...
We reconcile the two verdicts by naming the mechanism. The hand-built circuit points at memory with strongly correlated, "Gaussian" position keys; gradient descent instead settles on near-*orthogonal* keys — functionally identical, through different-looking weights. But the weight-space test is the wrong tool for the question: it flags even two independently-trained networks as different algorithms, so its verdict says nothing about whether they compute the same thing. The practical lesson is small and useful: look inside — trust the probe over the interpolation.
