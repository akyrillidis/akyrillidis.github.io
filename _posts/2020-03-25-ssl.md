...
where $\mathcal{X}$ is the labeled data 

$$
\{x_{i_1}, y_{i_1}\}_{i_1 = 1}^n,
$$

$\mathcal{U}$ is the unlabeled data $\{x_{i_2}^{u}\}_{i_2 = 1}^{n_u}$, $\mathcal{X}'$ and $\mathcal{U}'$ are the output samples labeled by MixMatch, and $E$, $A$, $\alpha$, $\lambda_3$ are hyperparameters. 
